...
3 full-sky-model: add option to enable "coordinates sync" in NINA (in order to make a real sync to the mount). Restore previous value after model build.
4 add option to use dedicated plate solve settings (exp.time, binning, gain, offset) for full-sky-model build and MLPT (2 setting groups)